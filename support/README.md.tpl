...
Micrate is primarily a library so a small piece of custom code is required to provide a minimal `micrate` executable. This is done by placing the following in `src/micrate.cr` (the example is for PostgreSQL but can trivially be adapted to MySQL or SQLite):
